...
def evaluate_patch_next_token_prediction_batch(
    ...
    def _evaluat_single_batch(batch_df):
        ...
        dist_orig = torch.softmax(
            output_orig.logits[np.array(range(batch_size)), position_source_batch, :],
            dim=-1,
        )
        ...
        hs_patch_config = [
            {
                ...
                "skip_final_ln": (
                    layer_source_batch[i] == layer_target_batch[i] == mt.num_layers - 1
                ),
            }
            for i in range(batch_size)
        ]
    ...
    for i in tqdm.tqdm(range(len(df) // batch_size)):
        ...
        batch_prec_1, batch_surprisal, batch_next_token = _evaluat_single_batch(cur_df)
...
def evaluate_attribute_exraction_batch(
    mt,
    df,
    batch_size=256,
    max_gen_len=10,
    ...
    def _evaluate_attribute_exraction_single_batch(batch_df):
        batch_size = len(batch_df)
        prompt_source_batch = np.array(batch_df["prompt_source"])
        prompt_target_batch = np.array(batch_df["prompt_target"])
        layer_source_batch = np.array(batch_df["layer_source"])
    ...
    for i in tqdm(range(len(df) // batch_size)):
        cur_df = df.iloc[batch_size * i : batch_size * (i + 1)]
        batch_results = _evaluate_attribute_exraction_single_batch(cur_df)
